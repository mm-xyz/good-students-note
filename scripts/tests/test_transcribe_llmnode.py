...
分段是為了長檔跑得完:長檔單次跑 whisper 會讓 ssh 連線撐不住(106 分鐘那集實測
失敗)。**零標點是另一件事,靠 --prompt 的寫法解,分段解決不了**(390 秒單段帶
同一份長 prompt 跑照樣 0 標點)——別把這兩件事綁在一起看。

plan_segments 的四個不變量都是產物正確性,不是美觀問題:
    覆蓋 — 漏一段就是逐字稿少一段話
    不重疊 — 重疊就是同一句被轉錄兩次
    不超上限 — 單段過長,ssh 斷線問題就回來了
