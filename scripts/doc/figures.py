...
輸出:
    <session_dir>/fig_p<頁碼3位>_<序2位>.png   — 渲染出的圖
    <session_dir>/doc_figures.json             — manifest(list[dict])
    stdout 最後一行：{"figures": N, "pages_scanned": M}
"""
...
# ════════════════════════════════════════════════════════════════════════════
#  常數（沿襲 vlm_prep.py 的篩選/偵測門檻）
# ════════════════════════════════════════════════════════════════════════════
DEFAULT_MIN_SIZE = 128        # 內嵌圖濾掉門檻：寬高較小邊 < 此值視為裝飾 icon/分隔線
...
# ════════════════════════════════════════════════════════════════════════════
def plan_items(doc, min_size: int) -> tuple[list[dict], bool]:
...
#  主流程
# ════════════════════════════════════════════════════════════════════════════
def process(input_path: str, session_dir: str, min_size: int = DEFAULT_MIN_SIZE,
           dpi: int = DEFAULT_DPI) -> dict:
    """跑完整渲染流程，回傳 stats dict {"figures": N, "pages_scanned": M}。
    ...
    try:
        ...
        return {"figures": len(manifest), "pages_scanned": pages_scanned}
    finally:
        doc.close()
...
def main() -> int:
    ...
    ap.add_argument("--min-size", type=int, default=DEFAULT_MIN_SIZE,
                    help=f"濾掉寬高較小邊 < 此值(px)的內嵌裝飾小圖/icon（預設 {DEFAULT_MIN_SIZE}）")
