...
**唯一的例外是 `## 🔇` 室噪留白**:每張 block 卡片底下有「＋ 之後留白」的秒數
按鈕(0.5／1.0／1.5／2.0),按下去會在那一句之後插入 `## 🔇 <秒數>  留白`;
插入的行會**顯示**在畫面上並帶「刪除」鈕,兩個動作都吃既有的復原。

為什麼只開這一個口:①「哪裡該留白」只有人審聽得出來,不是 render 算得出來的;
②既然人可以插,就必須看得到、也刪得掉,否則在手機上插錯沒有退路。
