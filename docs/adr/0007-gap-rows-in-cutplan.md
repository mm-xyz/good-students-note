...
## Amendment（2026-07-29 同日，MM 追加）

- **能量標注＋burst 拆分**：讀 audio16k 量每個 gap 的 30ms 窗 RMS——真靜音維持
  單列標「靜音」；**有聲的小段各自拆成獨立「🔊 聲音事件」列**（>-40dB 連續窗合併、
  ≥0.12s 成列、前後 pad 0.15s），勾選＝只保留那聲笑/打板/音效，不連帶死空白。
  「12% 有能量」語義＝時間比例（有聲窗數/總窗數）。
- 同一空白拆出多個 burst 時逐列並排（write/add-gaps 以 before 分組，防 dict 相撞
  吃列——EP16 實測 17:30 空白拆出兩事件）。
