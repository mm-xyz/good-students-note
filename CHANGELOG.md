...
- **遠端 ASR 改分段轉錄**（ADR-2026-09-07-asr-segmented-transcription）：
  ...
  一旦滑進「不標點模式」就自我延續到結束（批次 55 講實測 21 講整份零標點，而且非黑
  即白）。觸發條件是 prompt 的**書寫形式**：對照實驗顯示 21 字／62 字的自然敘述都是
  每 ~11.7 字一個標點，147 字的敘述＋長串專名列舉是 0 個。分段另外解掉「長檔單次跑
  ssh 斷線」（106 分鐘那集）。`transcribe_llmnode.py` 改成切 ~7 分鐘一段分別轉錄、每段重新套
