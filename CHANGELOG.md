...
- **新增 `scripts/audio/check_punct_density.py`——逐字稿標點密度稽核**：零標點是個
  沒有其他症狀的失敗（cue 數、字數、時間軸單調性、簡繁轉換全部正常），這支把它變成
  可自動化的 gate。判準是密度不是標點數（長短檔沒有可比性）；`--list` 印不合格的目錄名
  一行一個、可直接餵回 `batch_llmnode.sh` 重轉，`--check` 有不合格回 exit 1。
  三個會讓它自己算錯的地方都鎖了測試（17 支）：時間軸行的 `,`/`:` 不得混進標點數、
  半形逗號要算（whisper.cpp 逗號輸出半形）、`[講者]` 前綴不算字數。
  ⚠️ **連續標點算一個**——whisper 用 `...` 標停頓，逐字元算會讓 EP18 變成每 2.0 字
  一個標點，密度整個失真；數的是句界記號不是標點字元。
  實跑 `sessions/`：EP16 每 32.0 字、EP18 每 5.1 字通過，**EP15 是 11602 字 / 5 標點
  ＝每 2320 字，不合格**。

- **遠端 ASR 改分段轉錄**（ADR-2026-09-07-asr-segmented-transcription）：
  長檔單次跑 whisper 會讓 ssh 連線撐不住（106 分鐘那集實測失敗），切 ~7 分鐘一段、
  段落 JSON 的 `offsets` 平移回全域時間軸再合併，失敗只賠一段。
  ⚠️ **分段不是為了修零標點**：批次 55 講有 21 講整份零標點，成因是 `--prompt` 的
  寫法（對照實驗：21 字／62 字的自然敘述都是每 ~11.7 字一個標點，147 字的敘述＋長串
  專名列舉是 0 個，各跑兩次數字相同），把中招的音檔單獨拉出來 390 秒帶同一份長 prompt
  跑照樣 0 標點——分段解決不了，要改 context.txt 寫法。`transcribe_llmnode.py` 改成切 ~7 分鐘一段分別轉錄、每段重新套
  `--prompt`，段落 JSON 的 `offsets` 平移回全域時間軸再合併；新增
  `--segment-seconds`（420）/`--max-segment-seconds`（480），尾巴併進最後一段、
  併完超過上限就對半切。≤480s 的音檔行為不變（單段跑完）。
  補 `test_transcribe_llmnode.py`：覆蓋、不重疊、不超上限、無孤兒段四個不變量
  0.1s–6000s 全掃。`transcribe_local.py`（`--asr local`，剪輯線預設引擎）的 context
  註解同步改掉——原本寫「人名/專名放 context 開頭最有效」，那正是會壓掉標點的列舉形式。

- **修：分軌線的片尾音樂被塞進節目中央**（ADR-2026-09-07-structure-anchor-rewind）：
  MM 回報 EP18 v8「結尾的串場跑到中間去了」，互相關實測片尾音樂峰值在 8:54.1（全片
  18:47，相關 0.953）。分軌線把非 block 的結構行（🎵/🎬/章節）錨到「後面第一個逐軌
  block 的來源時間」，預設文件順序＝時間順序；cutplan 尾端的低信心 `<details>` 附錄會
  回頭重列早期 block，`## 🎵 ending` 排在附錄之前，於是錨到 `MR0018 [0:41]`。錨點規則
  加回跳守衛（block 時間早於目前最大值＝離開時間軸，待錨結構行改錨片尾並印警告），
  比的是最大值不是上一筆（附錄內部遞增，只比相鄰兩筆會在第二筆誤判回到正軌）。
  波及 final_cut_v7、v8 兩版（ending 行 2026-09-04 才加入）。
- **`structure_anchors()` 從 `main()` 抽成模組層純函式**：回傳 `(anchors, rewinds)`，
  警告文字留呼叫端。原本埋在 main() 中段只能靠端對端 dry-run 間接覆蓋，抽出後補了
  4 支回歸測試（附錄回跳、附錄第二筆不算回正軌、遞增節目單行為不變、G 列透明）。

## 2026-09-06

- **第三條剪輯路線：`## ⚙ audio=mixdown`**（ADR-2026-09-06-mixdown-audio-line）：
  剪輯決定照分軌人審跑完，音源改用混音 `source.wav`，不混 speech bus。MM 聽完 EP18 v7
  的結論是「分軌的品質不是很理想」，但 1515 個勾選、294 個靜音、16 處刪除線全長在逐軌
  節目單上，改走混音線等於把人審整批丟掉重來（混音 block 平均近 7s，用 migrate_marks
  以 0.6 覆蓋率門檻搬過去，細顆粒剪除會大量對不上）。分軌決定層的 segments 本來就長在
  `source.wav` 時間軸上，這條路線是**少做一步**而不是新演算法。
  代價寫明並由 render 印出來：分軌的「留住時間、只靜音某一軌」在合軌上做不到——EP18
  673 個 atomic cell 有 176 個屬此類、共 31.8s、占保留 2.9%，那些串音會回到成品。
  `audio=mixdown` 寫在混音線節目單上直接 FAIL。EP18 以本路線出 v8。

- **`cut.py` 不再用檔名猜剪輯路線**：舊判斷式 `args.plan != "cutplan.md"` 讓 EP18
  v4–v7 四份 render.txt 全標成「混音線」，同一份檔案上一行卻印著 `⚙ line=pertrack`。
  MM 因此以為混音線早就試過（「我記得已經有混音線的了，直接重跑不就好了」）。改為
  `route_label()` 從 ⚙ 讀，讀不到才照 block 前綴判（兩碼＝分軌、單碼＝混音）。

- **`semantic_diff` 補上路線改動**：它比的是 `params`，值限定 `[\d.]+`，`line=`／
  `audio=` 這種字串鍵完全不在裡面——Drive↔session 對照時換一條剪輯路線只印「內容有
  差異但不影響剪輯」。改比 `params_raw`。

## 2026-08-11

- **room-tone 鋪底的鬼影**（ADR-2026-08-11-672）：MM 實聽 EP18「一分鐘就聽不完」「聽起來是人聲」。
  真因不是底噪是鋪底本身——挑窗只排「三軌能量和最小」，EP18 挑出的 8 段有 7 段含
  呼吸／衣物／微弱人聲（短窗峰谷差 15.7–25.7dB），交叉淡接後循環鋪滿整集 →
  **週期 7.84 秒、峰值過完 loudnorm 是 −31dB，20 分鐘每輪逼聽一次**。預測事件落點
  37.2s 正好命中 MM 點名的 0:36–0:38。改成：挑窗要**同時安靜與平坦**（只看平坦度
  會挑到有人穩定講話的窗，實踩底噪 −60.9→−42.1dBFS 更糟），鋪底改**中位數幅度譜
  ＋隨機相位合成**（無事件、無週期）。EP18 實測 bed 峰谷差 23.5→3.3dB、週期相關
  0.77→0.12、底噪 −62.5dBFS。⚠️ 成品**整體**底噪未改善（P1 −46.8→−44.0），
  那是另一件事（見下）。
  - 派 Fable／codex luna／codex sol 三方評審方案，**三份都沒抓到真因**——我給的
    brief 把 room tone 用 RMS 判成「不是主因」，漏了峰值與週期性。

- **量到但還沒修：346 秒開麥空轉**（全片 30%）：每個 block 邊界的字前字後餘裕加總，
  Sarah 那支訊噪比最差的麥空轉最久（138s）。另有 35.4 秒是「被關掉的人真的在出聲、
  又沒有主聲遮蔽」的裸露串音。三份評審收斂的方向＝逐軌 downward expander（非硬 gate）
  ＋ `apply_mask` 的時間層與增益層解耦，收在 ADR-2026-08-11-672「尚未解決」。
