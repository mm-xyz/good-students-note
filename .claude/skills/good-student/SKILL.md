...
2. **視角（identity）**：記憶鉤要掛在使用者的哪個已知體系上？
   （例：八字命理背景學占星→全部往五行/十神/宮位對照類比；
   工程師學法律→往系統/介面/例外處理類比。）
   **每次啟動都要重新討論**——同一個人在不同材料上想用的視角可能不同；
   不問而沿用上次的視角＝違規。
...
- 驗收重點問使用者：「隨便挑一張，蓋住內文只看鉤子，你能複述這個概念嗎？」
...
- 委派 prompt **逐字**附上：frontmatter 範本＋內文格式＋該批視角＋受控詞彙表＋
  預期產出檔名清單。不留「格式自行調整」空間。
