...
中途失敗、或別人手動改過）。**開工前核對它聲稱的產物是否真的在**：

```bash
ls <session>/{cleaned_with_figs.md,figures.json,images} 2>/dev/null
```

不存在就當作沒做過，重新按流程走，並把 metadata 更正。
這跟 0-1「不能看副檔名推論有沒有圖」是同一種坑的另一種形式——
**相信別人寫的紀錄而不去驗檔案在不在**。

### 0-3c　**選定哪一張圖，要開圖看圖號，不能只憑錨點**

`score_figures.py` 是靠**圖周邊的文字**評分，相鄰兩張圖常共用同一段錨點
（實測 `023.jpg` 與 `024.jpg` 前後文幾乎一樣）。只憑錨點分不出是哪一張。

**開圖看圖上自己印的圖號**（「圖二十二」）再落筆。
沒看就照 score 排序寫下去，圖說內容可能對不上那張圖。
...
- **有工具，而且不用設定就能跑**：
  ```bash
  python3 scripts/check_layout.py <cleaned.md>            # 通用檢查，零設定
  python3 scripts/check_layout.py <corpus 根> --all       # 整批掃
  python3 scripts/check_layout.py <cleaned.md> --unit 宮  # 加驗表格系列
  ```
  **通用檢查永遠先跑**（孤字／圖連結斷鏈／同一張圖被插兩次），不需要領域知識。
  表格系列檢查（項數齊不齊、相鄰是否重疊）只有「多張圖湊成一組清單」的材料才適用——
  單張敘述型圖說沒有單位可比，**不必硬套**。不給 `--unit` 時它會自己偵測並提示要不要加。

  實測：2026-09-07 事故檔用 `--unit 宮 --expect 12` 抓出五張項數不齊、六組相鄰重疊；
  2026-09-08 的 merge off-by-one 則是通用檢查抓的（853 張中 407 處孤字）。
