...
title doc-vlm-to-md 安裝程式 v1.2.0
...
set "INSTALL_DIR=%USERPROFILE%\.doc-vlm-to-md"
set "SCRIPT_DIR=%~dp0"
...
if not exist "%SKILL_SRC%\scripts\requirements.txt" (
    ...
    set "EXTRACT_DIR=%TEMP%\doc-vlm-to-md-skill-%RANDOM%%RANDOM%"
    set "DVM_SKILL_ZIP=%SCRIPT_DIR%skill.zip"
    set "DVM_EXTRACT_DIR=!EXTRACT_DIR!"
    echo [準備] 解壓縮技能檔...
    powershell -NoProfile -ExecutionPolicy Bypass -Command "Expand-Archive -LiteralPath $env:DVM_SKILL_ZIP -DestinationPath $env:DVM_EXTRACT_DIR -Force" >nul
    if errorlevel 1 (
...
copy /Y "%SKILL_SRC%\scripts\kb_prep.py" "%INSTALL_DIR%\" >nul
...
if not exist "%INSTALL_DIR%\kb_prep.py" (
    echo    找不到已安裝的 kb_prep.py，請截圖回報老師。
    pause
    exit /b 1
)
...
(
echo @echo off
echo "%VENV_PY%" "%INSTALL_DIR%\kb_prep.py" %%*
) > "%INSTALL_DIR%\doc-vlm-to-md.bat"

echo.
echo 驗證安裝...
"%VENV_PY%" -c "import fitz, PIL; print('核心套件 OK')"
if errorlevel 1 (
    echo    套件匯入失敗。上方會顯示真正錯誤原因，請截圖回報老師。
    pause
    exit /b 1
)
"%VENV_PY%" "%INSTALL_DIR%\kb_prep.py" --help >nul
if errorlevel 1 (
    echo    kb_prep.py 驗證失敗。上方會顯示真正錯誤原因，請截圖回報老師。
    pause
    exit /b 1
)
...
echo 5. 確認 doc-vlm-to-md 出現在 Skills 列表中
echo.
echo 手動使用：
echo "%INSTALL_DIR%\doc-vlm-to-md.bat" --auto C:\Users\你的名字\Desktop\報告.pdf -o C:\Users\你的名字\Desktop\
echo.
pause
exit /b 0
