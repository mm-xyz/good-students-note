...
.venv-audio/bin/python scripts/frames/screen.py "<slug>"       # llm-node VLM 篩圖寫圖說
...
- **`frames/`**(2026-07-28 併入 invisible-context): `extract.py` 場景偵測抽幀、`screen.py` llm-node VLM 篩圖、`ocr.py` macOS Vision OCR+QR、`diagram.py` 圖→mermaid、`format_text.py` 版面重排、`compose.py` Obsidian 逐字稿+筆記(吸收音訊線的停頓/🔥/講者圖層);產物住 `sessions/<slug>/frames/`
